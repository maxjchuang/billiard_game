...
当前 Demo 已覆盖以下中式 8 球开局规则：

- 标准 `16` 球开局：`1` 白球、`7` 全色、`7` 花色、`1` 黑八
- 5 排三角球堆：顶角压置球点、黑八位于第三排中心、尾排两端异组
- 开球有效性判定：进目标球或至少 `4` 颗目标球碰库
- 开球犯规后续选择：线后自由球、要求重开、接受当前台面
- 首次合法入袋定组：同杆同时进全色与花色时等待玩家选择
- 黑八特殊处理：开球进黑八回置，未清台提前进黑八判负，清台后合法进黑八判胜
- Web 调试按钮：除开始/重开外，还支持开球犯规选项与混合进球选组
- Web 物理参数 HUD：支持在 `dev:web` 中实时修改摩擦、反弹、停球阈值、袋口吸附半径、库边厚度与最大出杆速度
...
- `openingBreakMinRailContacts`：有效开球所需最少碰库目标球数
- `openingBreakOptions`：开球犯规后的可选后续处理
...
4. `Esc` 或右键可取消当前蓄力
5. 右上角可点击 `重开`、`返回菜单`
6. 左上角点击 `展开物理 HUD`，可实时查看/修改物理参数并一键恢复默认

试玩时建议重点确认：

- 球桌外围有清晰的木质边缘（wood rails）
- 能一眼清点出 6 个球洞（4 角 + 2 边中），且不被 Web HUD/按钮遮挡
- 球进入可见洞口区域时，落袋判定与视觉位置一致
- 物理 HUD 默认折叠；展开后可修改参数、看到只读 `fixedDt` 诊断，并通过 `恢复默认` 快速回退
...
- 回合结算
- 占位渲染结构
- Web 调试物理参数 HUD（实时修改、校验、恢复默认、布局同步）
- 微信平台适配骨架
- 测试与构建链路
